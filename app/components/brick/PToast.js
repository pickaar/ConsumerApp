...
export const PToast = ({type,message, time, position}) => {
  Toast.show({
    type: type ? type : 'success', // <-- Type of toast (success, error, info, or custom)
    text1: message,
    position: position ? position : 'top', // <-- Must explicitly set the position
    visibilityTime: time === 'SHORT' ? 2000 : 4000,
    // You do NOT pass the offset here
  });

  // const toastConfig = {
  // warning: ({ text1, props }) => (
  //   <View style={{ height: 60, width: '90%', backgroundColor: 'gold', borderLeftColor: 'orange' }}>
  //     <Text>{text1}</Text>
  ...
  //  Toast.show({
  //     type: 'success', // 👈 Type of toast (success, error, info, or custom)
  //     text1: 'Order Confirmed! 🎉', // 👈 The main heading
  //     text2: 'Your items will be shipped within 24 hours.', // 👈 The optional message/body
  //     position: 'top', // 👈 Position (top or bottom)
  //     visibilityTime: 4000, // 👈 How long the toast is visible (4 seconds)
  //   });
  return null;
};
